...
* 测试方法：
  ...
  以上为基于开发工程师 commit 与 MR 的系统自动信息拉取与状态变更的完整流程。除此之外，在该功能需求关联的 Issue 被关掉后，对应的功能需求状态会从“测试中”变为“已交付”。具体详见视频展示。
...
* 测试方法：
  ...
  或者，更简便的方式，请点击左侧“功能需求管理”一栏，之后点击希望查看的功能需求标题，之后点击功能需求卡片。这时右侧栏中会有关联合并一栏，即可便捷查看关联的合并请求。
...
* 测试方法：
  ...
  我们为测试创建了一个项目，详情请见预置账户文档。我们创建的 cpython 项目对 cpython 仓库进行了爬取，进行压力测试，共有约 110k 个commit。

  后端使用多个uWSGI worker处理并发请求。我们使用多线程脚本并行请求测试，密集创建 2.4k 条 IR 以及 48k 条 SR，模拟多用户同时在线操作，后端工作良好。在上述负载下，各页面加载情况正常。
